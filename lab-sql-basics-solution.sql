...
-- subquery to display account_id with balance information
select account_id, (incoming, outgoing, incoming-outgoing) as total_balance
from 
	(select 
		(select account_id, round(sum(amount),0)
		where type = 'PRIJEM' and account_id = 396
		group by type) 
        as incoming, 
		(select account_id, round(sum(amount),0)
